...
class SimpleMLP(nn.Module):
    def __init__(self, in_dim, out_dim, hidden_ratio=2) -> None:
        super().__init__()
        self.mlp = nn.Sequential(
            nn.Linear(in_dim, int(hidden_ratio*in_dim)),
            nn.ReLU(True),
            nn.Linear(int(hidden_ratio*in_dim), in_dim)
        )
        self.fc = SimpleLinear(in_dim, out_dim)
    
    def forward(self, x):
        feat = self.mlp(x)
        out = self.fc(feat)
        out.update(feature=feat)
        return out

def get_freezed_network_copy(model):
